...
use App\Domains\Admin\User\DataTables\VerifiedMasterDataTable;
...
class VerifiedMasterController extends Controller
{
     ...
     */
    public function index(VerifiedMasterDataTable $dataTable)
    {

        abort_if(Gate::denies('verified_master_access'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        try {
            // dd("here");
            return $dataTable->render('User::verified-master.index');
        } catch (\Exception $e) {
            return abort(500);
        }
    }
    ...
    {
        abort_if(Gate::denies('verified_master_show'), Response::HTTP_FORBIDDEN, '403 Forbidden');
        if($request->ajax()) {
            try{
                $master = User::with(['roles'])->where('uuid', $id)->first();
                
                $viewHTML = view('User::verified-master.show', compact('master'))->render();
    ...
    // {
    //     abort_if(Gate::denies('verified_master_edit'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    //     try {
    ...
    //         $viewHTML = view('User::verified-master.edit', compact('master'))->render();
    ...
    // {
    //     abort_if(Gate::denies('verified_master_delete'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    ...
    //      abort_if(Gate::denies('verified_master_status'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    //     if ($request->ajax()) {
    ...
    //                 $response = [
    //                     'status'    => 'true',
    //                     'message'   => trans('cruds.verified-master.title_singular').' '.trans('messages.crud.status_update'),
    //                 ];
    //                 return response()->json($response);
    //             } catch (\Exception $e) {
    //                 DB::rollBack();
    ...
    //     abort_if(Gate::denies('verified_master_ban'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    //     if ($request->ajax()) {
    //         $validator = Validator::make($request->all(), [
    //             'id'     => [
    //                 'required',
    ...
    //     abort_if(Gate::denies('verified_master_is_approve'), Response::HTTP_FORBIDDEN, '403 Forbidden');
    //     if ($request->ajax()) {
    //         $validator = Validator::make($request->all(), [
    //             'id'     => [
    //                 'required',
    ...
    // }
       ...
       public function isVerifiedMasterApproved(Request $request){
            ...
            if (!$validator->passes()) {
                ...
                try{
                    ...
                    if($request->isApproved == 1){
                        $admin->update(['approval_status' => $request->isApproved, 'status' => 'active']);
                    }
                    else if($request->isApproved == 2){
                        $admin->update(['approval_status' => $request->isApproved, 'status' => 'inactive']);
                    }
                    DB::commit();
                    $response = [
                        'status'    => 'true',
                        'message'   => trans('cruds.verified_master.title_singular').' '.trans('messages.crud.status_update'),
                    ];
                    return response()->json($response);
                } catch (\Exception $e) {
                    DB::rollBack();
