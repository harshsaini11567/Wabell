...
@can('verified_master_edit')
    $(document).on("click", ".btnEditVerifiedMaster", function() {
        pageLoader('show');
    ...
    $(document).on('submit','#editVerifiedMasterForm', function(e) {
        e.preventDefault();
...
@can('verified_master_status')
    $(document).on('click','.verified_master_status_cb', function(){
        let $this = $(this);
        let userId = $this.data('verified_master_id');
        let flag = true;
        ...
        .then(function(result) {
            if (result.isConfirmed) { 
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('verified-masters.status') }}",
                    dataType: 'json',
...
@can('verified_master_ban')
$(document).on('click','.verified_master_isban', function(){
    let $this = $(this);
    let userId = $this.data('verified_master_id');
    ...
    .then(function(result) {
        if (result.isConfirmed) { 
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('verified-masters.isban') }}",
                dataType: 'json',
                data: { _token: csrf_token, id: userId },
                success: function (response) {
                    if(response.status == 'true') {
...
@can('verified_master_is_approve')
$(document).on('click','.verified_master_is_approved', function(){
    let $this = $(this);
    let userId = $this.data('verified_master_id');
    ...
    .then(function(result) {
        if (result.isConfirmed) { 
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('verified-masters.isapproved') }}",
                dataType: 'json',
...
@can('verified_master_show')
    $(document).on("click", ".btnViewVerifiedMaster", function() {
        let url = $(this).data('href');
        pageLoader();
...
@can('verified_master_delete')
$(document).on("click",".deleteVerifiedMasterBtn", function() {
        let url = $(this).data('href');
        Swal.fire({
            title: "{{ trans('global.areYouSure') }}",
...
$(document).ready(function () {
    ...
    $('#added_required_fields_users').on('change', function () {
        ...
        table.ajax.url("{{ route('verified-masters.index') }}?user_status=" + filterValue).load();
    });
});


$(document).on('click','.verified_master_approval_status', function(){
    let $this = $(this);
    let userId = $this.data('verified_master_id');
    let isApproved = $this.data('status');
    ...
    let confirmText = confirmTemplate
        .replace(':action', action)
        .replace(':user_type', 'verified master');
    ...
    .then(function(result) {
        if (result.isConfirmed) { 
            ...
            $.ajax({
                type: 'POST',
                url: "{{ route('verified-masters.approval-status') }}",
                dataType: 'json',
                data: { _token: csrf_token, id: userId , isApproved: isApproved},
                success: function (response) {
                    if(response.status == 'true') {
                        toasterAlert('success',response.message);
                        // $('#member-user-table').DataTable().ajax.reload(null, false);
                         $('#master-table').DataTable().ajax.reload(null, false);
                    }
                },
                error:function (response){
